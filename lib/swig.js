...
exports.Swig = function (opts) {
  ...
  this.parseFile = function (pathname, options) {
    ...
    if (options.resolveFrom) {
      if (fs.statSync(options.resolveFrom).isFile()) {
        pathname = path.resolve(path.dirname(options.resolveFrom), pathname);
      } else {
        pathname = path.resolve(options.resolveFrom, pathname);
      }
    }
